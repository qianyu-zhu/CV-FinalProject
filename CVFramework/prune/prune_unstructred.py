...
prune_method_bin = [prune.RandomUnstructured, prune.L1Unstructured]
# rune_method_bin = ['ln', 'random']

print('this is the trial for L1/random unstructred pruning, \
        prune encoder in np.linspace(0.0, 1.0, 9), \t\
        decoder in np.linspace(0.0, 1.0, 9)')
print('the matrix notation: first for random, second for L1, \t\
    ...
    for b in range(len(decoder_ratio_bin)):
        ...
        for c in range(9):
            ...
            for module, dummy in parameters_to_prune_encoder:
                # prune.ln_structured(
                #     module, 'weight', amount=0.3, dim=1, n=float('-inf')
                #     )
