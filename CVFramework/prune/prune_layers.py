...
layers = [1,2,3,4,5,6]
prune_method_bin = [prune.L1Unstructured]
# rune_method_bin = ['ln', 'random']

print('this is the trial for L1 unstructred pruning, \
        prune encoder in [0.1, 0.3, 0.5, 0.7, 0.9], \t\
        decoder in [0.1, 0.3, 0.5, 0.7, 0.9]')
print('the matrix notation: first for decoder 0.1, second for decoder 0.15, etc. \t\
        raw for encoder, colum for layers')
trial_out = np.zeros(5*5*6).reshape(5, 5, 6)

decoder_ratio_bin = [0.1, 0.3, 0.5, 0.7, 0.9]
prune_method = prune_method_bin[0]
...
for b in range(len(decoder_ratio_bin)):
    ...
    for c in range(5):
        amount = [0.1, 0.3, 0.5, 0.7, 0.9][c]
        print('encoder pruning ratio =: {:.2f}'.format(amount))
        ...
        for a in range(6):
            ...
            # for decoder, we use unstructred random/L1
            prune.global_unstructured(
            parameters_to_prune_decoder,
